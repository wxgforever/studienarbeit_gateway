...
DefinitionBank2Topic bank[100];/*in this programm there two threads.
This array of structure should be a global variant, so that both threads can use this array of structure*/
...
/**
 * @brief run a OPC UA Server
 * no parameter. The server is a global variant, so that both of the threads can set it.
 */
...
int main(int argc, char **argv) {
	...
	ros::NodeHandle n;
    signal(SIGINT, stopHandler);/*switch to turn off Gateway*/
    signal(SIGTERM, stopHandler);/*switch to turn off Gateway*/
...
// <<position to give topicname>>
	...
	char msgname[300];
	char msgtype[10240];
	std::string message[50]; /*array of string to save the definition of message type of ROS topic line by line*/

	/*set the structure "bank"
	 * treat the definition of message type of ROS Topic
	 */
	GetMsgfromTopic(topic,msgname);/*get name of ROS message type*/
	GetMsgDefbyMsgname(msgname,msgtype); /*get the definition of the message type*/
	GetMsgDef(msgname,message,&count); /*save the content of the definition line by line. Get the amount of parameters*/
	GetDefinitionBank2Topic(message,count,bank);/*set the definition into array of structure bank
	 bank is global variable*/
	GetNodeIdNameInBank(bank,count);/*set the nodeidname of parameters*/
    ...
    ros::spin(); /*switch to turn off ROS node*/

    thrd.join();/*turn off server thread*/
