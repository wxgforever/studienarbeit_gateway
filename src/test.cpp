...
static void makeOutputAssignment(ArgumentBank argumentbank,std::string outputassignment[]){
	...
	for(int i=0;i<argumentbank.count2outputargument;i++){
		...
		ss.clear();
		ss.str("");
		if(dataType == (&UA_TYPES[UA_TYPES_INT16])) ss<<"trans_bank.trans_int16=*(UA_Int16*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_INT32])) ss<<"trans_bank.trans_int32=*(UA_Int32*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_INT64])) ss<<"trans_bank.trans_int64=*(UA_Int64*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_BOOLEAN])) ss<<"trans_bank.trans_bool=*(UA_Boolean*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_BYTE])) ss<<"trans_bank.trans_byte=*(UA_Byte*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_STRING])) ss<<"trans_bank.trans_string=*(UA_String*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_UINT16])) ss<<"trans_bank.trans_uint16=*(UA_UInt16*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_UINT32])) ss<<"trans_bank.trans_uint32=*(UA_UInt32*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_UINT64])) ss<<"trans_bank.trans_uint64=*(UA_UInt64*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_FLOAT])) ss<<"trans_bank.trans_float=*(UA_Float*)input[";
		if(dataType == (&UA_TYPES[UA_TYPES_DOUBLE]))  ss<<"trans_bank.trans_double=*(UA_Double*)input[";
		ss<<i<<s1;
...
int main(void) {
	...
	std::string packagename="beginner_tutorials";
